...
export function ExperienceProvider({ children }: { children: ReactNode }) {
  ...
  // For logged-in users, derive focus from role if they haven't chosen one
  useEffect(() => {
    if (!user || !role) return;
    if (hasSelectedFocus) return; // user already chose — respect that

    const roleToFocus: Record<string, ActiveFocus> = {
      company: "enterprise",
      individual: "talent",
      expert: "consulting",
      student: "academy",
      instructor: "academy",
      admin: "explore", // admin goes to backoffice regardless
    };
    const derived = roleToFocus[role] ?? "explore";
    setActiveFocus(derived);
  }, [user, role, hasSelectedFocus]);
