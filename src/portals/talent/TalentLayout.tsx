/**
 * Talent Portal Layout — account-type-aware sidebar + account-type-first quick bar.
...
export default function TalentLayout({ children }: { children: ReactNode }) {
  ...
  const isCompanyWorkspace = accountType === "company" || accountType === "admin";
  const groups = isCompanyWorkspace ? companyGroups : freelancerGroups;
  const headerConfig = isCompanyWorkspace ? TALENT_COMPANY_HEADER : TALENT_FREELANCER_HEADER;
