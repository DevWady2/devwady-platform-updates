...
describe("AcademyDashboard router", () => {
  ...
  it("renders student dashboard for student account types", () => {
    mockUseAuth.mockReturnValue({ accountType: "student" } as any);
    renderWithSentinel();
  ...
  it("redirects instructor-only users to /instructor/workspace", () => {
    mockUseAuth.mockReturnValue({ accountType: "instructor" } as any);
    renderWithSentinel();
  ...
  it("preserves admin access to the student dashboard", () => {
    mockUseAuth.mockReturnValue({ accountType: "admin" } as any);
    renderWithSentinel();
    expect(screen.getByText("student-dashboard-view")).toBeInTheDocument();
  });
});
