...
describe("BackofficeDashboard", () => {
  ...
  it("renders overview stats, quick actions, and pending actions", () => {
    ...
    expect(screen.getByRole("link", { name: /New Quote/i })).toHaveAttribute("href", "/admin/quotes/new");
    expect(screen.getByRole("link", { name: /Review Accounts/i })).toHaveAttribute("href", "/admin/users");
    expect(screen.getByRole("link", { name: /Send Notification/i })).toHaveAttribute("href", "/admin/notifications");
  ...
  it("renders recent enterprise requests and quick links", () => {
    ...
    expect(screen.getByTestId("activity-feed")).toHaveTextContent("ActivityFeed 8");
    const quickLinks = screen.getAllByRole("link");
    const bookingsLinks = quickLinks.filter((l) => l.getAttribute("href") === "/admin/bookings");
    expect(bookingsLinks.length).toBeGreaterThanOrEqual(2);
    expect(quickLinks.some((l) => l.getAttribute("href") === "/admin/projects")).toBe(true);
    expect(quickLinks.some((l) => l.getAttribute("href") === "/admin/training")).toBe(true);
    expect(quickLinks.some((l) => l.getAttribute("href") === "/admin/users")).toBe(true);
  });
