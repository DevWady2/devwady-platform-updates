...
describe('route ownership — academy is student-only, instructor workspace is separate', () => {
  ...
  it('academy portal allowedRoles excludes instructor', () => {
    expect(PORTALS.academy.allowedRoles).toContain('student');
    expect(PORTALS.academy.allowedRoles).toContain('admin');
    expect(PORTALS.academy.allowedRoles).not.toContain('instructor');
  });

  it('instructor portal allowedRoles includes instructor', () => {
    expect(PORTALS.instructor.allowedRoles).toContain('instructor');
    expect(PORTALS.instructor.allowedRoles).toContain('admin');
    expect(PORTALS.instructor.allowedRoles).not.toContain('student');
  });
});
