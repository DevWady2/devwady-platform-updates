...
describe('auth and portal guards', () => {
  ...
  it('RoleGuard prefers canonical accountType for access decisions', () => {
    ...
    render(
      <MemoryRouter>
        <RoleGuard allowedAccountTypes={['admin']} allowedRoles={['admin']}>
          <div>Admin Only</div>
        </RoleGuard>
      </MemoryRouter>,
    );
  ...
  it('RoleGuard denies access when canonical identity does not match', async () => {
    ...
    render(
      <MemoryRouter initialEntries={['/admin']}>
        <Routes>
          <Route path="/admin" element={<RoleGuard allowedAccountTypes={['admin']} allowedRoles={['admin']}><div>Admin Only</div></RoleGuard>} />
