...
const mockState = {
  ...
  accountStatus: 'active' as 'pending_verification' | 'pending_approval' | 'active' | 'suspended' | 'banned' | 'deactivated',
...
describe('AuthProvider / useAuth', () => {
  beforeEach(() => {
    ...
    mockState.roles = [
      { role: 'individual', is_primary: true },
      { role: 'company', is_primary: false },
    ];
    mockState.accountStatus = 'pending_approval';
  ...
  it('hydrates canonical account state and keeps role/roles as a single-role compatibility shim', async () => {
  ...
  it('marks approved status only for active accounts with a canonical account type', async () => {
    mockState.roles = [{ role: 'company', is_primary: true }];
